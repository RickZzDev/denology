...
class FavoritedItemsView extends StatelessWidget {
  const FavoritedItemsView({Key? key, required this.products})
      : super(key: key);
        ...
        child: Column(
          children: [
            ...
            BlocBuilder<HomeViewModel, HomeState>(
              builder: (context, state) => products.onlyFavoritedList.isEmpty
                  ? const Padding(
                      padding: EdgeInsets.all(8.0),
                      child: Text(
                          "Você ainda não adicionou nenhum pedido favoritado :("),
                    )
                  : Expanded(
                      child: ListView.builder(
                        shrinkWrap: true,
                        itemCount: products.onlyFavoritedList.length,
                        itemBuilder: (context, index) => ProductCard(
                          item: products.onlyFavoritedList[index],
